...
const extraI18n = {
    en: {
        ...
        'faq.a5': "Manual takeoff means an estimator counts and measures bars by hand from the drawing. CAD detailing plugins automate detailing and BBS generation inside a CAD tool, working from a model that's already been drawn. AI rebar takeoff like SetTime instead reads the drawing itself — PDF or CAD — and detects bar sizes, spacing, and laps directly, producing quantities and a bar-bending schedule without a detailer building the model first.",
    ...
    ja: {
        ...
        'faq.a5': '手作業の積算は、担当者が図面から鉄筋を目視で数え、寸法を測るものです。CAD配筋プラグインは、すでに作成済みのモデルをもとにCADソフト内で配筋詳細図やBBSを自動生成します。SetTimeのようなAI鉄筋積算は、図面（PDFまたはCAD）そのものを直接読み取り、鉄筋のサイズ・間隔・重ね継手を検出することで、モデルを事前に作成することなく数量とBBSを算出します。',
    ...
    es: {
        ...
        'faq.a6': 'El despiece de acero con IA no es totalmente automático, y SetTime no afirma que lo sea. Los detalles congestionados, las reglas de empalme y solape, y las revisiones a mitad de proyecto siguen requiriendo el criterio de un estimador. La función de SetTime es acelerar el conteo y reducir errores de la lista de despiece — los estimadores con experiencia deben revisar igualmente el resultado antes de usarlo en una oferta o una orden de fabricación.',
        'nav.news': 'Noticias',
        'news.tag': 'Noticias',
    ...
    ko: {
        ...
        'faq.a5': '수기 산출은 견적 담당자가 도면에서 철근을 직접 세고 측정하는 방식입니다. CAD 배근 플러그인은 이미 작성된 모델을 기반으로 CAD 툴 안에서 배근 상세도와 절단·가공표(BBS)를 자동 생성합니다. SetTime과 같은 AI 철근 산출은 이와 달리 도면(PDF 또는 CAD) 자체를 직접 읽어 철근 규격, 간격, 이음 길이를 감지해, 별도로 모델을 만들지 않고도 수량과 절단·가공표를 산출합니다.',
